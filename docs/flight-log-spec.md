...
| 行った国 | 出発・到着空港の所在国のユニーク数。**乗り継ぎ含む／除くの2値を並記** |
| 最も多く行った国 | 訪問回数最大の国。同様に含む／除くの2基準で算出 |
...
- 統計5項目のみのカード（地球儀なし）を**iframe埋め込み**用に提供
...
- `airports.json`: iata, name, city, country_code, country_name, lat, lon（OpenFlightsから生成、リポジトリに同梱）
