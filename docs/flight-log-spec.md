...
| 総飛行時間 | 全フライトの飛行時間合計。`◯◯h◯◯m` 表示＋その下に小さくDays/Weeks/Months/Years換算を並記。便ごとの時間は実離着陸（`ops.actual_takeoff/landing`）→ ゲート実時刻 → 予定時刻の優先順で採用し、空港マスタのtz database名でローカル時刻をUTC換算して算出 |
| Top Airports | 空港別の発着回数ランキング（出発・実効到着それぞれ1カウント）。上位5件を表示 |
| Top Airlines | 航空会社別の搭乗便数ランキング。上位5件を表示 |
