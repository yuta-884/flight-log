# フライトログ Webアプリ 要件・仕様書（v0.16 確定版）

v0.15からの変更: OGP対応を追加。統計ページと埋め込みカードにOGP/Twitterカードのメタタグを設定し、`/assets/og-image.png`（1200×630、ヘッダー＋ルートマップ）を同梱。note.com等のiframe不可のプラットフォームではリンクカード（OGP）で共有する運用。og:imageは静的スナップショットで、更新したい場合は `/embed/stats.html?ogp=1`（撮影用表示モード）を1200×630でキャプチャして差し替える。
