# フライトログ Webアプリ 要件・仕様書（v0.14 確定版）

v0.13からの変更: Phase 4（埋め込みカード `/embed/stats.html`）を実装。Flightyのパスポートカードを参考にしたデザイン。§3.4に実装詳細を追記。
...
**実装詳細（v0.14で確定）**: Flightyの「My Flighty Passport」カードを参考にしたパスポート風デザイン
- 上段: 2Dワールドマップ（等長方形図法をcanvas描画。陸地は同梱GeoJSON、ルートは大圏をslerp近似、空港は赤ドット）＋訪問国の円形国旗バッジ列
- ミシン目区切りの下段: タイトル・便数・発行情報（Authority=flight-log / Place of issue=最多発着空港 / Date of issue=集計日 / Member since=初フライト日 ※`stats.json`の`first_flight_date`）・統計4項目（Distance / Flight Time（`◯d ◯h`表記）/ Airports / Airlines）・MRZ風装飾
