# フライトログ Webアプリ 要件・仕様書（v0.10 確定版）

v0.9からの変更: 地球儀上に、発着いずれか1件でも記録のある空港ごとにIATAスリーレターのラベルを常時表示（空港単位で1つ、フライトごとの重複なし）。
...
- 発着記録のある空港には**IATAスリーレターのラベルを空港ごとに1つ常時表示**（ドット表示とは別レイヤー。ホバーで発着数のツールチップ）
